...
function transact_one(url){
  ...
  $('.receipt_items').append('<tr class="receipt_total"><td><strong>Total</strong></td><td style="text-align:right"><strong>'+total+'</strong></td></tr>')
  ...
  $.ajax({
    type : 'post',
    data : {items:items, tender:tender, total:total},
    url : url,
    success: function(data){
      if (confirm('Transaction complete.\nPrint receipt?')){
        $('#full_receipt').printArea('popup')
...
function updateTotal(){
  ...
  /*if(count > 4 ){
      if(count > 4 && count <10){
        ...
        discounted = addDiscount($(this).data('value'), 10) 
      }
  }else{
