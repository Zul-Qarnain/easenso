...
def genEmpID(request):
    username = '%s' %genEmpUsername()
...
def validateUserLogin(request):
    ...
    if frmlogin.is_valid():
        ...
        if user is not None and user.is_active and hasAccess(user.id, 'employee'):
            login(request,user)
...
def employeeSearch(request):
    ...
    if user is not None and hasAccess(user['id'], 'employee'):
...
def employeeProfile(request):
    ...
    return render_to_response("security/profilepanel.html", data, context)
        ...
        if request.method == 'POST':
            ...
            if emp_form.is_valid():                
                if request.POST.get('action')=='add':
                    user = User(username = emp_form.cleaned_data['username'],
                                first_name = emp_form.cleaned_data['first_name'],
                                last_name = emp_form.cleaned_data['last_name'])
                    if User.objects.filter(first_name=user.first_name, last_name=user.last_name).exists():#employee already exist
                        data['error_message'] = "Employee '%s %s' Already Exist" %(user.first_name, user.last_name)
                        data['form'] = emp_form
                        ...
                        user.save()
                        user.groups.add(emp_form.cleaned_data['group'])
                        employee = Employee(user = user, position=emp_form.cleaned_data['position'], address = emp_form.cleaned_data['address'])
                        if 'image_path' in request.FILES:
                            employee.image_path = request.FILES['image_path']
                        employee.save()
                        data['success_message'] = "New Employee Succesfully Saved"
                        data['form'] = emp_form
                        return render_to_response('security/addedit_emp.html', data, context)
                else:#edit
                    pass
